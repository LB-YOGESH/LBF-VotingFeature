...
import { ApiPut, ApiGet } from "../../Helpers/Api/ApiData";
import {
...
const ChatContainer = ({
  ...
  const Answer = (answer) => {
    ...
    if (answer?.createdBy?._id === userData?._id) {
        ...
        <>
          <div className="answer-grid-items">
            <div className="answer-profile-image">
              {answer?.createdBy && answer?.createdBy?.profileImage ? (
                <img src={answer?.createdBy?.profileImage} />
              ) : (
                <img src={ProfileImg} />
              )}
            </div>
          ...
          <div className="answer-grid-items">
            <span style={{ fontWeight: 600 }}>
              {answer?.createdBy &&
                answer?.createdBy?.currentRole &&
                answer?.createdBy?.currentRole}
            </span>
            <span>
              {answer?.createdBy &&
              answer?.createdBy?.subject &&
              displayProfile === true
                ? answer?.createdBy?.subject
                : "Anonymous"}
            </span>
      ...
      return (
        <>
          <div className="answer-grid-items">
            ...
            {console.log("========Answer ===========", userData)}
            {userData?.votedAnswers.includes(answer?._id) ? (
              <div className="votes">
                <img src={upVoteImg} className="vote-icon voted" />
                <div className="rating" id={`rating${answer?._id}`}>
                  {answer?.rating}
                </div>
                <img src={downVoteImg} className="vote-icon voted" />
              </div>
            ) : (
              <div className="votes">
                <img
      ...
      answer?.requestBy?._id == userData?._id &&
      ...
      answer?.requestTo?._id == userData?._id &&
  ...
  const voteAnswer = async (f, answer) => {
    let up, down;
    ApiGet(`answer/vote/id=${answer?._id}`)
      .then((res) => {
        if (res?.data?.result === 1) {
          if (f === 1) {
            up = answer?.upVote + 1;
            down = answer?.downVote;
            document.getElementById(`rating${answer?._id}`).innerHTML = `${
              answer?.rating + 1
            }`;
          } else {
            up = answer?.upVote;
          ...
          userData.votedAnswers.push(answer._id);
    ...
    <div className="test-question-grid-items">
      <div className="text-send-box">
                        ...
                        answer?.status === "pending" && (
                          <>
                            <div className="mb-allow-reject-grid">
                              ...
                              <div className="mb-allow-reject-grid-items">
                                <p>
                                  You received a request for accessing your
                                  profile
                                </p>
                                <span>
                                  {moment(answer?.createdAt).fromNow()}
        ...
        <div className="" style={{ position: "relative" }}>
          <div
            className="who-can-see-alignment"
            onClick={() => setWhoCan(!whoCan)}
          >
            <div>
              <svg
                viewBox="0 0 24 24"
          ...
          <div
            className={
              whoCan
                ? "who-can-drop who-cand-show"
                : "who-can-drop who-cand-hidden"
            }
          >
