...
const MyQuery = () => {
  ...
  const handleSubjectSelect = (sub, id) => {
    if (selectedTopSubjects?.find((ss) => ss === sub?.subject)) {
      setSelectedTopSubjects((curVal) =>
        curVal.filter((cv) => cv !== sub?.legendsName)
      );
    } else {
  ...
  const searchQuestionData = () => {
    if (answerLaterData) {
      if (inputValue) {
        // console.log("Answer Later =============>", inputValue);
          ...
          if (f === 1) {
            ...
            document.getElementById(`rating${allQuestion?._id}`).innerHTML = `${
              allQuestion?.rating - 1
									...
									userData.votedQuestions.push(allQuestion?._id);
                  Auth.setUserData(userData);
        }
        ApiPut(
          `question/vote/id=${allQuestion?._id}&downVote=${down}&upVote=${up}`
        )
          .then((res) => {
            if (res.data.result === 0) {
        ...
        <div className="answer-later-design">
          <div className="white-modal-design">
            <div className="modal-body">
              <p>
                <p>
                  The Response Count is the number of Public and
                  Author-View-Only responses. Only Public Responses are Visible
                  to you.
                </p>
              </p>
            </div>
